"""Main trading simulator orchestrator."""
...
from src.trading.types import Order, Fill, OrderSide
...
class TradingSimulator:
    ...
    def __init__(self, venues: List[str], symbols: List[str]):
        ...
        logger.info(f"Trading simulator initialized",
                   venues=len(venues), symbols=len(symbols))
    ...
    async def simulate_trading(
        ...
        logger.info(f"Starting trading simulation for {duration_seconds}s")
        ...
        async for tick in market_data_generator.generate_market_data_stream(
            ...
            market_state[symbol_venue_key] = {
                ...
                'spread_bps': (
                    (tick.ask_price - tick.bid_price) / tick.mid_price * 10000
                ),
                'volume': tick.volume,
            ...
            if tick_count % 1000 == 0:
                ...
                logger.verbose(f"Processed {tick_count} ticks in {elapsed:.1f}s",
                             total_pnl=f"${self.total_pnl:.2f}")

        results = self._generate_simulation_results()

        logger.info("Simulation complete",
                   ticks=tick_count, trades=self.trade_count)
    ...
    async def _execute_order(
        ...
        if order.predicted_latency_us:
            latency_noise = np.random.normal(
                0, order.predicted_latency_us * 0.1
            )
            actual_latency_us = max(
                50, order.predicted_latency_us + latency_noise
            )
        else:
            ...
            if strategy_name == 'market_making':
                ...
                strategy_results = {
                    **pnl_data,
                    ...
                    'capture_rate': (
                        strategy.opportunities_captured /
        ...
        if self.performance_history:
            ...
            if len(returns) > 0:
                results['risk_metrics'] = {
                    ...
                    'max_drawdown_pct': (
                        self.max_drawdown / max(abs(self.high_water_mark), 1) * 100
                    ),
    ...
    def _analyze_ml_impact(self) -> Dict[str, float]:
        ...
        for fill in ml_routed_fills:
            order = next(
                (o for o in self.order_history if o.order_id == fill.order_id),
                None
            )
        ...
        return {
            ...
            'avg_latency_prediction_error_us': (
                np.mean(latency_errors) if latency_errors else 0
            ),
