...
class ArbitrageStrategy(TradingStrategy):
    ...
    async def generate_signals(
        ...
        for symbol, prices_by_venue in venue_prices.items():
            ...
            best_bid = max(
                prices_by_venue.items(), key=lambda x: x[1].get("bid_price", 0)
            )
            best_ask = min(
            ...
            if bid_data["bid_price"] > ask_data["ask_price"]:
                ...
                if arb_bps >= self.params["min_arb_bps"]:
                    ...
                    buy_size = min(
                        ask_data.get("ask_size", 100), self.params["max_position"]
                    )
                    sell_size = min(
                        bid_data.get("bid_size", 100), self.params["max_position"]
                    )
